...
koaRouter.post("/sentVerifyCode", async (socket: ParameterizedContext): Promise<void> => {
	...
	const verifyCode: string = GenerateRandomCaptcha.generate(target.session);
	try {
		if (emailRegex.test(target.email)) {
			await emailService.service.sendMail({
				from: emailService.config.username,
				to: target.email,
				subject: "服务器验证码",
				html: `您好 ${target.username} 玩家, 您的白名单验证码是 [${verifyCode} (10分钟有效)]`
			});
